...
import { isSapSqlServerConfigured, querySapSales } from "@/lib/sap-sqlserver";
import { mapSapRowToSales } from "@/lib/sap-mappers";

/** GET: SAP SQL Server 직접 조회 후 Supabase UPSERT. 미설정 시 SYNC_SALES_URL 사용. */
export async function GET() {
  try {
    if (isSapSqlServerConfigured()) {
      const rows = await querySapSales();
